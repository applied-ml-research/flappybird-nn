...
def eval_nn_1(nn, gm):
  ...
  return bool(evaluation[1] > evaluation[0])
...
def train_nn_1(nn, reward=1.0, death_reward=-100.0, reward_decay=0.9, sample_ratio=0.3):
  ...
  step(-1, torch.FloatTensor([death_reward, death_reward]).to(DEVICE))
  for idx in sample:
    step(idx, reward + reward_decay * nn['memory'][idx + 1])
...
def run(file, nn_params={'init': init_nn_1, 'eval': eval_nn_1, 'train': train_nn_1}, save_every=100, until=10000):
  ...
  for i in range(1, until + 1):
    ...
    while game_continues:
      ...
      game_continues = gm.update(flying)
